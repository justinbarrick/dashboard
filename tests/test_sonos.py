...
@with_sonos()
@with_client
async def test_sonos_zones(client, server, widgets):
    s = Sonos(client)
...
@with_sonos()
@with_client
async def test_sonos_widget_stopped(client, server, widgets):
...
@with_client
async def test_sonos_widget_music(client, server, widgets):
    response = await request_widget(client, 'sonos')
    assert_equal(response.json(), { "speakers": [ {
        "name": "Living Room",
        "state": {
...
@with_client
async def test_sonos_widget_tv(client, server, widgets):
    response = await request_widget(client, 'sonos')
    assert_equal(response.json(), { "speakers": [ {
        "name": "Living Room",
        "state": {
...
@with_sonos(speaker(state="PLAYING", tv=True))
@with_client
async def test_sonos_widget_pause(client, server, widgets):
...
@with_sonos(speaker(state="PLAYING", tv=True))
@with_client
async def test_sonos_widget_play(client, server, widgets):
...
@with_client
@with_external_call
async def test_sonos_widget_party_mode(fifo, client, server, hue_server, widgets):
...
@with_sonos(speaker(state="PLAYING", tv=True))
@with_client
async def test_sonos_widget_hue_integration(client, server, hue_server, widgets):
...
@with_sonos(speaker(state="PLAYING", tv=True))
@with_client
async def test_sonos_widget_hue_integration_tv(client, server, hue_server, widgets):
    widgets.wc.party_mode = True
...
@with_sonos(speaker(state="PLAYING", tv=True))
@with_client
async def test_sonos_widget_hue_integration_pause(client, server, hue_server, widgets):
    widgets.wc.party_mode = True
